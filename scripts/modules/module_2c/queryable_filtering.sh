...
#melyik térképet akarod lekérdezmi?
    ...
    Send_Message l 1 module_test.1 input actions [\"OK\"] $MODULE/vector_map_list
        Request
...
#a választott térkép melyik mezőjét?
    ...
    cat $GEOSERVER/cityapp/geoserver_data/$PRI_ID/$SEC_ID/$TERC_ID/$MAP_TO_QUERY".txt" | grep ":y;" | cut -d":" -f2 | sed s'/ //'g > $MODULE/queryable_fields
    
    Send_Message l 2 module_test.2 input actions [\"OK\"] $MODULE/queryable_fields
        Request
            FIELD_TO_QUERY=$REQUEST_CONTENT
            
#Melyik térképpel szűrjünk?
    Send_Message l 3 module_test.3 input actions [\"OK\"] $MODULE/vector_map_list
        Request
...
#E térkép melyik adatával?
    ...
    SEC_ID=$(echo $MAP_TO_QUERY | cut -d"_" -f2)
    TERC_ID=$(echo $MAP_TO_QUERY | cut -d"_" -f3)
    cat $GEOSERVER/cityapp/geoserver_data/$PRI_ID/$SEC_ID/$TERC_ID/$MAP_TO_FILTER".txt" | grep ":y;" | cut -d":" -f2 | sed s'/ //'g > $MODULE/filter_fields
    
        Send_Message l 4 module_test.4 input actions [\"OK\"] $MODULE/queryable_fields
            Request
                FIELD_TO_FILTER=$REQUEST_CONTENT
                
    echo "$MAP_TO_QUERY , $FIELD_TO_QUERY , $MAP_TO_FILTER , $FIELD_TO_FILTER" > $MODULE/test.txt
