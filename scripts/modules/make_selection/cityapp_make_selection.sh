...
# ---------------------
    ...
    # Do you really want to select a new area?
    Send_Message m 1 make_selection.1 question actions [\"Yes\",\"No\"]
        Request
...
#
    ...
    grass $GRASS/$MAPSET --exec v.extract input=lines type=line where="highway>0" output=highways --overwrite --quiet
   ...
   # grass $GRASS/$MAPSET --exec r.out.gdal input=m1_time_map output=$GEOSERVER/m1_time_map.tif format=GTiff type=Float64 --overwrite --quiet
    ...
    Send_Message m 4 make_selection.4 question actions [\"OK\"]
        Request
