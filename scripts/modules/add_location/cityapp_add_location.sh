...
GEOSERVER=~/cityapp/geoserver_data/base_maps
...
# -- Processing ------------------------
#
    # Do you really want to set a new location?
    ...
    if [ $(grass -f ~/cityapp/grass/global/PERMANENT/ --exec g.list type=vector | grep lines_osm) ]
        then
                ...
                Request
                    if [ "$REQUEST_CONTENT" = "yes" -o "$REQUEST_CONTENT" = "Yes" -o "$REQUEST_CONTENT" = "YES" ]
                        then
                            ...
                            Send_Message m 4 add_location.4 question actions [\"OK\"]
                                Request
                                    Running_Check stop
                                    Close_Process
                                    exit
            ...
            Send_Message m 3 add_location.3 question actions [\"Yes\",\"No\"]
                Request
                    if [ "$REQUEST_CONTENT" = "yes" -o "$REQUEST_CONTENT" = "Yes" -o "$REQUEST_CONTENT" = "YES" ]
                        then
                            INIT=1
...
case $INIT in
    "1")
        ...
        Send_Message m 5 add_location.5 upload actions [\"OK\"]
            Request_Map osm
...
Send_Message m 6 add_location.6 question actions [\"OK\"]
coordinates
