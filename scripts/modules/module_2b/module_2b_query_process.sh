...
if [ -e $MODULE/ward_id ]
    then
        WARD="$(cat $MODULE/ward_id)"   
        INIT=1
    else
        INIT=0
fi

#zenity --info --text="init: $INIT"
    ...
    # If any of basemaps are missing, importing 
        ...
        MAP_TO_QUERY_SLUMS=$(cat $MODULE/query_defaults | head -n6 | tail -n1)
        MAP_TO_QUERY_WARDS=$(cat $MODULE/query_defaults | head -n8 | tail -n1)
     
  
        if [ ! $(g.list type=vector | grep $MAP_TO_QUERY_HOUSEHOLDS | grep -v grep) ]
            then
                v.import input=~/cityapp/geoserver_data/bhubaneshwar/bbswr_slum_houses_ownership.gpkg output=$MAP_TO_QUERY_HOUSEHOLDS --overwrite
        fi

        if [ ! $(g.list type=vector | grep $MAP_TO_QUERY_OWNERSHIP | grep -v grep) ]
            then
                v.import input=~/cityapp/geoserver_data/bhubaneshwar/land_owners.gpkg output=$MAP_TO_QUERY_OWNERSHIP --overwrite
        fi
   
        if [ ! $(g.list type=vector | grep $MAP_TO_QUERY_SLUMS | grep -v grep) ]
            then
                v.import input=~/cityapp/geoserver_data/bhubaneshwar/slum_areas.gpkg output=$MAP_TO_QUERY_SLUMS --overwrite
        fi
        
        if [ ! $(g.list type=vector | grep $MAP_TO_QUERY_WARDS | grep -v grep) ]
            then
                v.import input=~/cityapp/geoserver_data/bhubaneshwar/wards.gpkg output=$MAP_TO_QUERY_WARDS --overwrite
        fi


        # creating empty maps (to avoid complaining the ps.map bcause of the lack of maps)
        
        v.edit tool=create map=government_land
        v.edit tool=create map=private_land
        v.edit tool=create map=temple_land
        v.edit tool=create map=nodata_land
...
Process_Check start calculations
    ...
    #First, setting the region to the entire calculation area
        ...
        case $INIT in
            0)
                Request_Map geojson
                zenity --info -text="input: $REQUEST_PATH"
                v.in.ogr -o input=$BROWSER/selection.geojson output=query_area_1 --overwrite
                Gpkg_Out query_area_1 query_area_1
                QUERY_AREA_1=query_area_1
                ;;
            1)
                v.extract input=$MAP_TO_QUERY_WARDS where="WARD_ID='$WARD'" output=query_area_1 --overwrite
                QUERY_AREA_1=query_area_1
                Gpkg_Out query_area_1 query_area_1
                ;;
        esac
    ...
    # The result depends on the INIT value.
