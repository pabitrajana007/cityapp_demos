...
############################################################################################################
    ...
    # Message 2 Maps of PERMANENT mapset can only be queryed. Default maps and "selection" map is not included in the list. Only map with numeric column (except column "CAT") will listed. What is the map you want to query? Available maps are:
        ...
        Send_Message l 2 module_2.2 select actions [\"OK\"] $MODULE/temp_maps
            Request
                ...
                if [ grass -f $GRASS/$MAPSET --exec g.list type=vector mapset=module_2 | grep "$MAP_TO_QUERY_1" ]
                    then
                        echo ""
                    else
                        grass -f $GRASS/$MAPSET --exec g.copy vector=$MAP_TO_QUERY_1"@"PERMANENT,$MAP_TO_QUERY_1
                fi
    ...
    # Message 3 Fill the form and press save.
        ...
        grass -f $GRASS/$MAPSET --exec db.describe -c table=$MAP_TO_QUERY_1 | grep -E 'DOUBLE\ PRECISION|INTEGER' | grep -vE 'CAT|cat' > $MODULE/temp_columns_1
...
#####################################################################################
    ...
    # Set region to query area, set resolution
        grass -f $GRASS/$MAPSET --exec g.region vector=$MAP_TO_QUERY_1 res=$QUERY_RESOLUTION --overwrite

    # Set MASK to query area
        grass -f $GRASS/$MAPSET --exec r.mask vector=$QUERY_AREA_1 --overwrite

    # Clip the basemep map by query area
        grass -f $GRASS/$MAPSET --exec v.select ainput=$MAP_TO_QUERY_1 atype=point,line,boundary,centroid,area binput=$QUERY_AREA_1  btype=area output=clipped_1 operator=overlap --overwrite
