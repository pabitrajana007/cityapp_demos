...
# Processing ----------------------------------------
    ...
    # connecting from/via/to points to the clipped network, if neccessary. Via points are optional, first have to check if user previously has selected those or not.
        if [ $VIA -eq 1  ]
            then
                ...
                g.rename  vector=m1a_highways_from_points,m1a_highways_points_connected --overwrite
        fi
    ...
    # Fill this new avg_speed column for each highway feature. Values are stored in $VARIABLES/roads_speed
        if [ ! -f $VARIABLES/roads_speed ]
            then
                cp $VARIABLES/roads_speed_defaults $VARIABLES/roads_speed --overwrite
            fi
    ...
    # zones:
        ...
        if [ $VIA -eq 1  ]
            then
                ...
                g.copy  vector=$FROM_POINT,m1a_from_via_points --overwrite
        fi
...
# Map output into a pdf file ----------------------
    ...
    # Generating pdf output
        ...
        # set color for maps:
            ...
            # by this:
            r.colors map=m1a_time_map rules=~/cityapp/scripts/modules/module_1a/color_rules
