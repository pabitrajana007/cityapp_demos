...
# Do you really want to add nem map(s) to the mapset?
Send_Message m 1 add_map.1 question actions [\"Yes\",\"No\"]
    Request
...
if [ ! $(grass $GRASS/$MAPSET --exec g.list type=vector | grep selection) ]
    then
        # Message 2 "Selection" map not found. Before adding a new layer, first you have to define a location and a selection. For this end please, use Location Selector tool of CityApp. Add_Map modul now quit.
        Send_Message m 2 add_map.2 error actions [\"Ok\"]
            Request
                Send_Message m 5 add_map.5 question actions [\"OK\"]
                    Request
                        Running_Check stop
                        Close_Process
                exit
