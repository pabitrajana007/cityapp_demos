...
# Preprocesing -----------------------------
    ...
    # Default constants values for the interpolation and time calculations. Only modify when you know what is the effects of these variables ---
        ...
        v.select ainput=m1b_input_points binput=selection@PERMANENT output=m1b_points --overwrite 
...
# Processing ----------------------------
    ...
    for i in $(v.db.select map=$FROM_POINT columns=cat where="cat>0" | grep -v cat);do
    ...
    r.mapcalc expression="m1b_accessibility_map_temp=0" --overwrite
    for i in $(g.list type=raster pattern=m1b_cost_*);do
        ...
        r.mapcalc expression="m1b_accessibility_map_temp=m1b_reclassed_$i+m1b_accessibility_map_temp" --overwrite
    done
        r.reclass input=m1b_accessibility_map_temp output=m1b_accessibility_map rules=$MODULE/reclass_rule_3 --overwrite
        # r.mapcalc expression="m1b_accessibility_map=m1b_accessibility_map_temp*10" --overwrite
        r.mapcalc expression="m1b_accessibility_map=m1b_accessibility_map*1" --overwrite
...
# Pdf and gpkg output ---------------------------------
  ...
  # Converting the result into vector point format
        ...
        v.out.ogr -s input=$FROM_POINT type=point output=$GEOSERVER/m1b_points.gpkg --overwrite
...
exit
        ...
        # Interpolating accessibility map for estimating covered population
        r.reclass input=m1b_temp output=m1b_zerospeed_temp rules=$MODULE/reclass_rule_2 --overwrite;
        r.mapcalc expression="m1b_zerospeed = m1b_zerospeed_temp * 1" --overwrite;
        r.mapcalc expression="m1b_accessibility_full = m1b_accessibility_map + m1b_zerospeed" --overwrite;
        r.surf.idw input=m1b_accessibility_full output=m1b_interpolated_accessibility --overwrite;
